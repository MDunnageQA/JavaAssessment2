...
public class Questions {
	...
	public String zipped(String input1, String input2) {
		...
		int trueLength = length1 + length2;
		char[] charArray = new char[trueLength];
		
		while ( i < input1.length())
		{
			charArray[k] = input1.charAt(i);
			k++;
			while (j < input2.length())
			{
				charArray[k] = input2.charAt(j);
				k++;
				j++;
				break;
			}
		...
		String stringToReturn =  new String(charArray);
	...
	public String[] removeDuplicates(String[] stringArray) 
	{
		...
		String[] noDups = new String[arrayLength];
	...
	public int getVowel(String input) 
	{
		...
		{
			if (input.charAt(i) == aA[0] || input.charAt(i) == aA[1])
			{
				vowelCounter++;
			}
			if (input.charAt(i) == eE[0] || input.charAt(i) == eE[1])
			{
				vowelCounter++;
			}
			if (input.charAt(i) == iI[0] || input.charAt(i) == iI[1])
			{
				vowelCounter++;
			}
			if (input.charAt(i) == oO[0] || input.charAt(i) == oO[1])
			{
				vowelCounter++;
			}
			if (input.charAt(i) == uU[0] || input.charAt(i) == uU[1])
			{
				vowelCounter++;
			}
		}
	...
	public boolean wordFinder(String initialWord, String madeString) 
	{
		...
		{
			if (charMatcher == madeString.length())
			{
				break;
			}
			...
			{
				if (madeString.charAt(i) == initialWord.charAt(j))
				{
					charMatcher++;
				}
				if (charMatcher == madeString.length())
